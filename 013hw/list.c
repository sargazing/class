...
void _Reverse_list(node_t *head, node_t *new2)
{

	new2->next = head->next;
	head->next = new2;
}
void reverse_list(listhead_t *head)
{
	node_t *next;
	node_t *buf = head->head.next;
	while(buf->next)
	{
		next = buf->next->next;
		_Reverse_list(&(head->head), buf->next);
		buf->next = next;
	}
}

void _list_des(node_t *p)
{
	int size = sizeof(*p);
	node_t *buf = p->next;
	...
	if(buf != NULL)
	{	
		memcpy(p->data, buf->data, sizeof(*p->data));
		//memcpy(p, buf, size);
		p->next = buf->next;
	}
	free(buf->data);
